...
interface RankUnit { unitId: string; centerX: number; width: number; height: number; topY: number; }
interface DragSlot { rank: number; insertIndex: number; x: number; y: number; width: number; height: number; }
...
export function useCardDrag(
  ...
  const dragStateRef = useRef<CardDragState | null>(null); // NOU — sursa de adevăr sincronă
  ...
  const toWorldY = useCallback((clientY: number) => {
    const rect = containerRef.current?.getBoundingClientRect();
    if (!rect) return 0;
  ...
  const startDrag = useCallback((unitId: string, clientX: number, clientY: number) => {
    ...
    dragStateRef.current = initial; // NOU — sincron, disponibil imediat
    setDragState(initial);
  }, [layout, buildUnitsByRank, toWorldX, toWorldY]);

  const updateDrag = useCallback((clientX: number, clientY: number) => {
    const prev = dragStateRef.current; // NOU — citim din ref, nu din closure de state
    if (!prev) return;
    ...
    dragStateRef.current = next; // NOU — actualizăm ref-ul sincron
    setDragState(next);
  }, [toWorldX, toWorldY, layout.direction, layout.maxRank]);

  const endDrag = useCallback(() => {
    document.body.style.cursor = '';
    const ds = dragStateRef.current; // NOU — mereu ultima valoare, fără race condition
    if (!ds) return;

    const others = (unitsByRankRef.current.get(ds.slot.rank) ?? [])
      .filter((u) => u.unitId !== ds.unitId)
      .map((u) => u.unitId);
    others.splice(ds.slot.insertIndex, 0, ds.unitId);

    dragStateRef.current = null;
    setDragState(null);
    onDrop(ds.slot.rank, others, ds.unitId, ds.sourceRank);
  }, [onDrop]); // stabil — nu se recreează la fiecare mișcare
